...
def test_go_to_market_ships_exactly_its_one_artifact():
    ...
    when a user asks"), now that #609 has registered it: go-to-market owns exactly `gtm_brief`, and
    ...
    assert get_perimeter(GO_TO_MARKET).artifact_types == frozenset({"gtm_brief"})
    slots = artifact_slots(GO_TO_MARKET)
    assert set(slots) == {"gtm_brief"}
    allowed, _ = schema_slot_ids(GO_TO_MARKET)
    assert slots["gtm_brief"] == set(allowed)
    assert "business_rules" not in slots["gtm_brief"]  # software-only, must not leak in
...
def test_the_go_to_market_artifact_generates_saves_and_goes_stale_end_to_end():
    ...
    assembly or `GoToMarketBrief`'s slot-vocabulary validator at all.
    ...
    result = disco.generate(meta.slug, "gtm_brief")
    ...
    assert rec.prompt_version == prompt_version("gtm_brief", perimeter=GO_TO_MARKET)
    assert rec.prompt_version != prompt_version("gtm_brief")  # software default -- must differ
    ...
    assert svc.meta(meta.slug).artifact_status["gtm_brief"].stale is True
...
def test_impact_on_capacity_reaches_the_go_to_market_artifact():
    ...
    binding-constraint impact default, and `artifact_slots(GO_TO_MARKET)["gtm_brief"]` is `"*"`
    ...
    report = svc.impact(slug, ["capacity"])
    assert report.changed == ["Capacity"]
    assert "gtm_brief" in report.artifacts
