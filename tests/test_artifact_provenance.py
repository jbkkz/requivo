...
def moved(tmp_path, monkeypatch):
    """A session at revision 2 whose `workflow` slot."""
...
def test_a_stated_source_revision_still_records_the_flag_it_always_did(moved):
    """MUST FIRE. The positive control for every refusal below."""
    svc = ArtifactService()
...
def test_an_omitted_source_revision_is_refused_rather_than_read_as_now(moved):
    """#6 F1. The save that used to be recorded `revision: 2, stale: false` about content the caller never
    said anything about."""
...
def test_the_refusal_says_what_to_pass(moved):
    """A refusal a caller cannot act on is a worse outcome than the wrong answer it replaced."""
    with pytest.raises(UnstatedSourceRevisionError) as e:
...
def test_a_refused_save_writes_nothing_at_all(moved):
    """The refusal happens before the artifact file and before `session.json` is touched."""
    svc = ArtifactService()
...
def test_a_corrupt_revision_file_is_refused_as_a_structured_error(moved):
    """#6 F2. `except RequivoError` only caught a *missing* revision."""
...
def test_a_missing_revision_file_is_still_refused_the_way_it_always_was(moved):
    """The case the original guard did catch, kept as a control."""
...
def test_the_two_provenance_refusals_carry_two_codes_and_one_details_shape(moved):
    """Two refusals share one `details` shape but ride two codes now (#57)."""
    svc = ArtifactService()
    ...
    # The two codes differ, and each is named rather than only compared.
    assert unstated.value.code == "unstated_source_revision"
    assert unreadable.value.code == "unreadable_source_revision"
    # The split moved the code, not the hierarchy — `except InvalidSessionError` still catches both, which is what keeps this from breaking a caller that catches the class.
    assert isinstance(unstated.value, InvalidSessionError)
    assert isinstance(unreadable.value, InvalidSessionError)
    # must fire: neither arm may answer the family base (#57).
...
def test_an_intact_history_is_still_diffed_rather_than_refused(moved):
    """MUST FIRE for the whole F2 block. Every test above breaks a revision file and asserts a refusal."""
