...
`scripts/check_dco.py` is the whole of the gate `.github/workflows/dco.yml` runs, so a test that only asserted "a signed branch passes" would be green against a script that always passes -- which is the failure mode a provenance check least survives. Every row below builds a real repository in a temp
directory and runs the real function over it: no mocked git, because what is under test is what git reports, and a stub would be asserting this file's own idea of `--format`."""
...
def repo(tmp_path):
    ...
    `user.name`/`user.email` are set on the repository rather than inherited, so this test asserts the same thing on a maintainer's machine and on a runner with no global git identity."""
    path = tmp_path / "repo"
    path.mkdir()
...
def test_a_commit_message_cannot_split_itself_into_a_second_record(repo, monkeypatch, capsys):
    ...
    Sign-off was still flagged correctly; this pins that a contributor cannot write a line of the CI log."""
    base = _git(repo, "rev-parse", "HEAD")
    ...
    captured = capsys.readouterr()
    combined = captured.out + captured.err
    assert sha in combined  # without this, the assertions below pass on a run that reported nothing
...
def test_a_merge_commit_is_reported_as_not_checked_rather_than_passed_over(repo, monkeypatch,
                                                                          capsys):
    """`--no-merges` is right about the metadata and wrong to leave implicit: a merge that resolved a conflict carries a tree diff of its own that no parent certifies. This repository requires a linear history so such a commit cannot reach `main` -- what the receipt must not do is claim *every commit carries a
    sign-off* after declining to look at one."""
    base = _git(repo, "rev-parse", "HEAD")
