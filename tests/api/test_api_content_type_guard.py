...
`docs/decisions/0004-the-http-api-facade.md` §5): every unsafe method requires `Content-Type:
application/json`, refused with 415 otherwise."""
...
def test_a_bodyless_post_still_needs_the_header(raw_client):
    """The rule is unconditional across every unsafe method, including one with no request model today (`POST
    .../discover`)."""
...
def test_a_post_with_the_json_content_type_is_not_refused(raw_client):
    """The must-not-fire control: the header this guard actually asks for is accepted."""
...
def test_a_get_needs_no_content_type(raw_client):
    """The must-not-fire control on the method axis."""
    resp = raw_client.get("/api/v1/sessions")
    assert resp.status_code == 200


def test_a_rebound_host_is_refused_before_the_content_type_check(raw_client):
    """Ordering, not reach: both guards refuse, but a request from a host the allowlist rejects is told 403
    `host_not_allowed` and not 415."""
