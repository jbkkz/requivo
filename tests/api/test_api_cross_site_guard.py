...
`docs/decisions/0004-the-http-api-facade.md` §5) -- `requivo.host_policy.check_request_origin`, the one
definition `web/security.py`'s `_enforce` also calls, wired by `api/app.py`'s `cross_site_guard`."""
...
def test_a_cross_site_fetch_is_refused(raw_client, site):
    """Must-fire: the browser's own account of the request's origin says elsewhere -> 403 `cross_site_fetch`,
...
def test_a_loopback_origin_is_accepted_whatever_its_spelling_or_port(raw_client, origin):
    """The must-not-fire control on the origin axis, and #43's own case."""
...
def test_no_origin_at_all_is_a_scripted_client_and_is_accepted(raw_client):
    """The asymmetry `check_request_origin` states: a browser attaches `Origin` to every POST."""
    resp = raw_client.post("/api/v1/sessions", content=BODY, headers=JSON)
    assert resp.status_code == 201


def test_a_read_is_never_attributed(raw_client):
    """The method axis: the checks run on unsafe methods only, so a cross-site *read* is not refused here (the
    host allowlist is what guards reads, and it ran already)."""
...
def test_a_rebound_host_is_refused_before_the_origin_checks(raw_client):
    """And the host guard still runs first of all (§5's ordering constraint)."""
    resp = raw_client.post("/api/v1/sessions", content=BODY,
...
def test_the_web_surface_calls_the_same_definition():
    """The sharing claim, mechanically: `web/security.py` re-exports these names from `host_policy` rather
    than defining its own -- identity, not equality."""
