...
def _with_brief(client, with_provider):
    """A discovered session with a decision brief saved, through the web."""
    with_provider(engine_reply(converged=True, problem=HIGH_EXPLICIT), BRIEF_REPLY)
    create_via_post(client)
    client.post(f"{_SESSION}/artifacts/brief")
...
def test_create_only_then_run_discovery(client, with_provider):
    ...
    assert r.status_code == 303 and "Awaiting analysis" in client.get("/sessions/leave-only").text   # no LLM yet
...
def test_a_hand_crafted_answers_post_on_a_revision_zero_session_is_refused(client, with_provider):
    ...
    r = client.post("/sessions/bare-rev0/answers", data={"answers": "…", "expected_revision": "0"})
...
def test_a_stale_answers_form_is_refused_before_the_provider_is_paid(client, with_provider):
    ...
    fake = with_provider(_ASKING)
    create_via_post(client)
    spent_on_discovery = len(fake.calls)
    r = client.post(f"{_SESSION}/answers", data={"answers": "…", "expected_revision": "999"})
    assert r.status_code == 409                          # RevisionConflictError → clean 409, not a traceback
    assert len(fake.calls) == spent_on_discovery, "provider call(s) were billed for a turn whose conflict was certain"


def test_a_matching_answers_form_still_reaches_the_provider(client, with_provider):
    """The must-fire half of the test above (#205): the swapped body leads with the impact of the answers."""
    fake = with_provider(_ASKING, _CONVERGED)
    create_via_post(client)
    spent_on_discovery = len(fake.calls)
    r = client.post(f"{_SESSION}/answers", data=_ANSWER)
    assert r.status_code == 200 and "What changed" in r.text
    assert len(fake.calls) == spent_on_discovery + 1, "a form rendered at the current revision has to reach the provider"
...
def test_generate_brief_and_prd_and_view(client, with_provider):
    ...
    create_via_post(client)
    assert client.post(f"{_SESSION}/artifacts/brief").status_code == 200
    assert client.post(f"{_SESSION}/artifacts/prd").status_code == 200
    page = client.get(_SESSION).text
...
def test_a_saved_artifact_reads_as_a_document_not_as_source(client, with_provider):
    ...
    _with_brief(client, with_provider)
...
def test_downloading_an_artifact_still_serves_the_bytes_that_were_saved(client, with_provider):
    """Rendering is a *view*. The file is the artifact, and it is what the reader hands on (#235)."""
    _with_brief(client, with_provider)
    saved = ArtifactService().show("leave-approval", "brief")
    assert client.get(f"{_SESSION}/artifacts/brief?download=1").text == saved
    assert "# Decision Brief" in saved, "must fire: the saved file really is Markdown"
...
def test_related_change_marks_artifact_stale(client, with_provider):
    # A PRD consumes business_rules; a material change to business_rules makes it stale.
    with_provider(_ASKING, PRD_REPLY, _CONVERGED)
    create_via_post(client)
    client.post(f"{_SESSION}/artifacts/prd")
    client.post(f"{_SESSION}/answers", data={"answers": "explicit now", "expected_revision": "1"})
    assert "Needs updating" in client.get(_SESSION).text
...
def test_the_web_offers_every_artifact_the_service_can_generate(client, with_provider, monkeypatch):
    ...
    assert client.post(f"{_SESSION}/artifacts/criteria").status_code == 200
...
def test_the_two_former_analyses_are_generatable_and_an_unknown_type_still_is_not(client, with_provider):
    ...
    with_provider(stories, stories, json.dumps({"items": [{"story_id": "S1", "title": "Request leave", "complexity": "S",
                                                            "days_low": 1, "days_high": 2}]}))
    _make_session("leave-approval", problem=HIGH_EXPLICIT)
    assert client.post(f"{_SESSION}/artifacts/risk-register").status_code == 400
    assert client.post(f"{_SESSION}/artifacts/stories").status_code == 200
    assert client.post(f"{_SESSION}/artifacts/estimate").status_code == 200
...
def test_an_unrelated_change_leaves_a_document_alone(client, with_provider):
    ...
    with_provider(engine_reply(problem=HIGH_EXPLICIT, reporting={"completeness": 10, "confidence": "empty", "impact": "low"}),
                  PRD_REPLY,
                  engine_reply(converged=True, problem=HIGH_EXPLICIT,
                               reporting={"completeness": 80, "confidence": "explicit", "impact": "low"}))
    create_via_post(client)
    client.post(f"{_SESSION}/artifacts/prd")
    r = client.post(f"{_SESSION}/answers", data={"answers": "Reporting is a weekly export.", "expected_revision": "1"})
    assert "Needs review" not in r.text and "Needs updating" not in client.get(_SESSION).text


def test_a_changed_answer_moves_the_scope_and_the_brief(client, with_provider):
    """The canonical scope-change story, end to end."""
    with_provider(engine_reply(problem=HIGH_EXPLICIT, integrations={"completeness": 40, "confidence": "inferred", "impact": "high",
    ...
    create_via_post(client, slug="leave-migration", request_text="Migrate leave to the new HR system")
    ...
    r = client.post("/sessions/leave-migration/answers",
                    data={"answers": "The migration is one-time — the legacy system becomes read-only.", "expected_revision": "2"})
...
def test_a_resolved_session_can_still_be_refined(client, with_provider):
    ...
    with_provider(engine_reply(converged=True, problem=HIGH_EXPLICIT), _CONVERGED)
    create_via_post(client)
    page = client.get(_SESSION).text
    ...
    r = client.post(f"{_SESSION}/answers", data={"answers": "One more thing — contractors are out of scope.", "expected_revision": "1"})
    assert r.status_code == 200 and "What changed" in r.text
