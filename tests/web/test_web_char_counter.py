...
# The harness declares this ceiling on its fields.
_HARNESS_LIMIT = 20_000


def test_the_harness_still_speaks_the_products_own_ceiling():
    """The literal strings the harness test asserts on ("16,000 / 20,000 characters") are only about the
    product while the harness declares the product's own ceiling."""
...
def test_the_character_counter_counts_and_warns_without_ever_touching_the_text():
    """The whole rule, on the shipped asset. The two assertions that carry it are the negative ones."""
    t = _counter_timeline()

    # Below the threshold the affordance says nothing at all.
    ...
    # The ceiling is the maximum *permitted* length, not the first refused one.
    ...
    # Deleting text takes the counter away again: the state is read off the field every time, never latched.
    assert t["back down again"]["text"] == ""
    assert t["back down again"]["className"] == "counter"

    # A field the page did not have at load, arriving through a swap with the reader's refused text already in it (#30).
    swapped = t["swapped in, already full"]
    assert swapped["text"] == "19,000 / 20,000 characters"

    # ── a ceiling the page cannot read ────────────────────────────────────────
    # The third state of the affordance, and the one the guard `!(limit > 0)` exists for.
    ...
    # must fire, for that whole block: every one of those fields holds 19,000+ characters, which on a readable ceiling is well past the threshold and demonstrably does produce a counter (the "swapped in, already full" row above, same length, same code path).
...
def test_the_limit_the_page_shows_is_the_limit_the_server_refuses_on(client, with_provider):
    ...
    # Positive control first: an errored or empty render satisfies every "in" assertion below by rendering no field at all, and the silence would read as a pass.
