"""Requivo Web security: response headers, the disk cache, the slug guard and redirect safety (#555)."""
...
# The 500 page was the one response class this app served with none of the above (#340).
#
# So this asserts the composition rather than a list of names.
#
# `content-length` and `content-type` are in the compared set on purpose.
...
# ── the disk cache (#218) ─────────────────────────────────────────────────────
#
# Where the boundary is drawn, and why it is not the content type.
#
# So the rule is the other way round and fails closed.

def test_no_response_carrying_the_readers_material_may_be_written_to_the_disk_cache(
        client, with_provider):
    """Every page, fragment and download this app answers with carries the reader's own request, the model
    built from it, or the request token."""
    ...
    rows = {
        ...
        # The row the comment above is *about*.
        "artifact download": client.get("/sessions/leave-approval/artifacts/brief?download=1"),
    }
    for name, response in rows.items():
        # Must fire: a 404 or a 500 would satisfy an assertion about a header just as happily as the real page, and the whole row would then be about an error page nobody caches anyway.
...
def test_slug_traversal_is_rejected(client):
    # A dot-segment slug never resolves to a path; the guard raises invalid_slug (400) or the route simply does not match (404) — either way, nothing outside the store is reached.
...
#
# CodeQL's py/url-redirection flags all four RedirectResponse call sites that concatenate a slug onto a fixed literal prefix (#500): sessions.py's analysis_failed, discovery.py's run_discovery and submit_answers, and artifacts.py's generate_artifact.

# `_FORM_FIELD_SLUG_ATTEMPTS` is for the one site whose slug is a `Form` field (`create_session`) rather than a `{slug}` path parameter: a form value is never subject to a client's own URL-path normalisation, so every one of these reaches `validate_slug` unchanged -- verified directly against `create_app()`, each returns 400 `invalid_slug`.
...
# `_PATH_PARAM_SLUG_ATTEMPTS` is for the three sites whose slug is a `{slug}` path parameter (`Depends(safe_slug)`).
...
def test_the_answers_redirect_never_leaves_this_origin_under_a_hostile_slug(raw_client):
    """discovery.py:121 -- the no-JS (non-htmx) branch of `submit_answers`."""
...
def test_the_generate_artifact_redirect_never_leaves_this_origin_under_a_hostile_slug(raw_client):
    """artifacts.py:60 -- the no-JS branch of `generate_artifact`, same reason `raw_client` is used above
    rather than `client`."""
    raw_client.headers[CSRF_HEADER] = csrf_token()
    for attempt in _PATH_PARAM_SLUG_ATTEMPTS:
...
def test_the_create_session_failure_redirect_never_leaves_this_origin_under_a_hostile_slug(client):
    """`sessions.py:109` -- `analysis_failed`'s own redirect, reachable only via `create_session`'s
    `Form`-field `slug`, validated before the redirect fires."""
...
def test_a_legitimate_slug_still_redirects_where_it_should(raw_client, with_provider, monkeypatch):
    """Must-not-fire control for every test above: a guard refusing a hostile slug means nothing next to proof
    an honest slug still reaches the redirect at all four sites."""
    raw_client.headers[CSRF_HEADER] = csrf_token()

    # discover (discovery.py:48): a fresh create_only session, at the revision-0 discovery is required to run from.
