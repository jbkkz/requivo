...
_HEADERS = ("Content-Security-Policy", "X-Content-Type-Options", "Referrer-Policy")


def _client(app, host=_LOOPBACK, *, token=False):
...
def _redirected(r, location: str) -> bool:
    return r.status_code == 303 and r.headers["location"] == location


def _boom(self, slug, *, surface="discover"):
...
def test_security_headers_present(client, path, expected_status):
    ...
    assert "default-src 'self'" in r.headers["Content-Security-Policy"] and "Referrer-Policy" in r.headers  # (#47)
...
def test_the_500_page_carries_every_header_an_ordinary_page_carries(client, failing_route):
    ...
    assert not set(ordinary.headers) - set(unhandled.headers)
...
def test_no_response_carrying_the_readers_material_may_be_written_to_the_disk_cache(client, with_provider):
    ...
    s = f"/sessions/{_make_session()}"
    rows = {"home": client.get("/"), "session page": client.get(s),
            "htmx fragment": client.post(f"{s}/answers", data={"answers": "Contractors are out.", "expected_revision": "1"}),
            "generation": client.post(f"{s}/artifacts/brief"), "model download": client.get(f"{s}/export"),
            "artifact page": client.get(f"{s}/artifacts/brief"), "artifact download": client.get(f"{s}/artifacts/brief?download=1")}
    for name, r in rows.items():
        # Must fire: an error page would satisfy a header assertion just as happily as the real page.
        assert r.status_code == 200, f"the {name} row never reached the response it is about"
        assert r.headers.get("Cache-Control") == "no-store", f"the {name} response may be disk-cached"


@pytest.mark.parametrize("path", ["/static/css/app.css", "/static/js/app.js", "/static/vendor/htmx.min.js", "/favicon.ico"])
def test_a_bundled_asset_stays_cacheable(client, path):
    ...
    r = client.get(path)
    assert r.status_code == 200 and "Content-Security-Policy" in r.headers, f"{path}: the middleware never ran"
    assert "no-store" not in r.headers.get("Cache-Control", ""), f"{path} must stay cacheable"
...
_FORM_FIELD_SLUG_ATTEMPTS = ["../etc", "../../etc/passwd", "%2e%2e%2fetc", "%2e%2e%252fetc", "..%5cwindows",
                             "/etc/passwd", "....//etc", "C:\\Windows", "evil.com", "@evil.com", "evil.com%2f%2f"]
_PATH_PARAM_SLUG_ATTEMPTS = ["a..b", "..%5cwindows", "C:\\Windows", "evil.com", "@evil.com", "leave%2Dapproval%00",
...
_REDIRECT_SITES = {
    "discover": lambda c, s, **kw: c.post(f"/sessions/{s}/discover", follow_redirects=False),
    "answers": lambda c, s, **kw: c.post(f"/sessions/{s}/answers", data={**_ANSWERS, **kw}, follow_redirects=False),
    "generate": lambda c, s, **kw: c.post(f"/sessions/{s}/artifacts/brief", follow_redirects=False),
    "create": lambda c, s, **kw: c.post("/sessions", data={**_CREATE, "slug": s, **kw}, follow_redirects=False),
}


@pytest.mark.parametrize("site", sorted(_REDIRECT_SITES))
def test_a_redirect_never_leaves_this_origin_under_a_hostile_slug(app, site):
    """Each of the four flagged `RedirectResponse` sites, under every attempt its slug's route lets through (#500)."""
    c = _client(app, token=True)
    for attempt in _FORM_FIELD_SLUG_ATTEMPTS if site == "create" else _PATH_PARAM_SLUG_ATTEMPTS:
        r = _REDIRECT_SITES[site](c, attempt)
        # A hostile slug is refused, never redirected: a 3xx here would already be off-site.
        assert r.status_code >= 400, f"slug {attempt!r} was not refused: {r.status_code} -> {r.headers.get('location')!r}"


def test_the_redirect_refusal_is_the_slug_guard_and_not_merely_a_missing_session(app):
    """The must-fire half: a 404 from a nonsense slug proves nothing."""
    for site in _REDIRECT_SITES.values():
        r = site(_client(app, token=True), "Not_A_Slug")
        assert r.status_code == 400 and "invalid_slug" in r.text, r.text


def test_a_legitimate_slug_still_redirects_where_it_should(app, with_provider, monkeypatch):
    ...
    assert _redirected(_REDIRECT_SITES["discover"](c, "leave-only"), "/sessions/leave-only")
    _make_session("leave-approval")
    assert _redirected(_REDIRECT_SITES["answers"](c, "leave-approval", expected_revision="1"), "/sessions/leave-approval")
    with_provider(BRIEF_REPLY)
    assert _redirected(_REDIRECT_SITES["generate"](c, "leave-approval"), "/sessions/leave-approval")
    monkeypatch.setattr(DiscoveryService, "run_discovery", _boom)
    r = _REDIRECT_SITES["create"](c, "leave-failed", provider="anthropic")
...
def test_a_reserved_slug_already_on_disk_is_reachable_through_the_web_read_routes(client, with_provider):
    ...
    assert _redirected(client.post("/sessions/con/discover", follow_redirects=False), "/sessions/con")
    assert store.read_meta("con").current_revision == 1 and client.get("/sessions/con/export").status_code == 200
    # Must-not-fire control: `POST /sessions` takes its slug from a form field and stays strict (#372).
    created = client.post("/sessions", data={**_CREATE, "slug": "nul"})
...
def test_the_token_works_as_a_form_field(raw_client, client):
    ...
    assert r.status_code == 303 and csrf_token() in client.get("/").text   # …and every rendered form carries it
...
def test_a_token_this_server_cannot_compare_is_refused_rather_than_crashing(raw_client):
    ...
    assert all(h in hostile.headers for h in _HEADERS), "the refusal escaped the header middleware"
    ...
    ok = raw_client.post("/sessions", data={**_CREATE, "slug": "still-works", CSRF_FIELD: csrf_token()}, follow_redirects=False)
...
def _enforce_at_the_seam(headers, *, app=None, method="POST", path="/sessions", receive=_empty_receive):
    """`_enforce` on a raw ASGI scope, where a header the framework cannot decode actually arrives."""
    ...
    return asyncio.run(_enforce(Request(scope, receive)))
...
def test_a_latin1_token_header_reaches_the_refusal_rather_than_the_comparison(app):
    ...
    with pytest.raises(MissingRequestTokenError):
        _enforce_at_the_seam([(b"host", b"127.0.0.1:8765"), (b"content-length", b"0"), (b"x-csrf-token", b"\xe9")], app=app)
...
def test_a_chunked_body_is_refused_before_being_read(app):
    ...
    async def instrumented_receive():
        ...
        return {"type": "http.request", "body": b"x" * 1_000, "more_body": len(receive_calls) <= MAX_BODY_BYTES // 1_000 + 1}

    headers = [*_POST_HEADERS, (b"x-csrf-token", csrf_token().encode())]
    with pytest.raises(InputTooLargeError):
        _enforce_at_the_seam(headers, app=app, receive=instrumented_receive)
    assert receive_calls == [], "the body was read before the missing Content-Length was refused"
    with pytest.raises(InputTooLargeError):   # keyed on the header, not on whether bytes actually follow
        _enforce_at_the_seam(headers, app=app)
...
def test_a_declared_length_post_is_unaffected(app):
    ...
    r = _client(app, token=True).post("/sessions", data={**_CREATE, "slug": "declared-length-still-fine"}, follow_redirects=False)
...
def test_a_write_from_another_origin_is_refused(client):
    assert client.post("/sessions", data=_CREATE, headers={"Origin": "http://evil.example"}).status_code == 403


def test_a_browser_declared_cross_site_write_is_refused(client):
    assert client.post("/sessions", data=_CREATE, headers={"Sec-Fetch-Site": "cross-site"}).status_code == 403
...
def test_a_request_addressed_to_another_host_is_refused(app):
    ...
    assert _client(app, "evil.example").get("/").status_code == 403
...
def test_a_host_the_server_cannot_determine_is_refused_rather_than_skipped(app):
    ...
    assert c.get("/").status_code == 200 and c.get("/", headers={"Host": "localhost:8765"}).status_code == 200  # must pass


def _verdict(headers: list[tuple[bytes, bytes]]) -> str:
    """`"accepted"`, or the refusal's error **code** (#52)."""
    try:
        _enforce_at_the_seam(headers, method="GET", path="/")
...
def _guard_post(app, *, host: str, slug: str, headers: dict | None = None):
    ...
    return _client(app, host, token=True).post("/sessions", data={**_CREATE, "request_text": "A leave approval system.", "slug": slug},
...
# are one origin (#43), `Referer` is the fallback the same line reads, no origin at all is the token's job,
# and an origin carrying userinfo is refused (#51).
_ORIGIN_VERDICTS = [
    ...
    ("userinfo-origin", _LOOPBACK, {"Origin": "http://evil.com@127.0.0.1:8765"}, None, 403),
...
def test_the_opaque_origin_is_refused_deliberately_and_says_which_arm_fired(app):
    ...
    assert _guard_post(app, host=_LOOPBACK, slug="attributed", headers={"Origin": "http://localhost:8765"}).status_code == 303
...
def test_the_policy_this_app_sends_and_the_origin_guard_it_runs_agree(app, client):
    ...
    assert policy in _ORIGIN_A_BROWSER_SENDS_ON_A_SAME_ORIGIN_FORM_POST, f"{policy!r}: add what Fetch says it does"
    origin = _ORIGIN_A_BROWSER_SENDS_ON_A_SAME_ORIGIN_FORM_POST[policy].replace("SELF", f"http://{_LOOPBACK}")
...
def test_two_hostnames_nobody_could_determine_are_not_a_match():
    ...
    pairs = [("", ""), ("", "127.0.0.1"), ("127.0.0.1", ""), ("localhost", "127.0.0.1"), ("evil.example", "evil.example"),
             ("evil.example", "127.0.0.1")]
    assert [_same_trust_domain(a, b) for a, b in pairs] == [False, False, False, True, True, False]
...
def test_a_cross_port_loopback_origin_is_accepted_and_that_is_the_decision(app):
    ...
    assert _guard_post(app, host=_LOOPBACK, slug="cross-port-loopback", headers={"Origin": "http://localhost:3000"}).status_code == 303
    assert _guard_post(app, host=_LOOPBACK, slug="cross-port-hostile", headers={"Origin": "http://evil.example:8765"}).status_code == 403
...
def test_api_key_never_reaches_the_browser(client, monkeypatch):
    ...
    assert not any("sk-secret-sentinel-123" in client.get(p).text for p in ("/", "/sessions/new", "/sessions/leave-approval"))
...
# ── #51: an authority the parser cannot determine a host from, and the ones it can (the must-fire half) ──

_AUTHORITIES = [
    ("evil.com@127.0.0.1", ""), ("http://evil.com@127.0.0.1", ""), ("user:pass@127.0.0.1", ""), ("@127.0.0.1", ""),
    ("127.0.0.1 evil.com", ""), ("127.0.0.1@", ""), ("[::1", ""), ("http:///", ""), ("", ""), ("   ", ""),
...
@pytest.mark.parametrize("value, expected", _AUTHORITIES)
def test_an_authority_that_is_not_a_host_is_refused_by_the_parser(value, expected):
    """Userinfo, whitespace, an unbalanced bracket or nothing at all parse to `""`; an ordinary authority still parses."""
...
def test_userinfo_no_longer_smuggles_a_loopback_host_past_the_allowlist():
    ...
    assert _hostname("evil.com@127.0.0.1") not in allowed_hosts() and _hostname("127.0.0.1") in allowed_hosts()
...
@pytest.mark.parametrize("listed, host", [(None, "127.0.0.1:8765"), ("requivo.example.test", "requivo.example.test")],
...
def test_the_guard_refuses_a_host_header_carrying_userinfo(app, monkeypatch, listed, host):
    ...
    c = _client(app, host)
    assert c.get("/", headers={"Host": f"evil.com@{host}"}).status_code == 403
    assert c.get("/", headers={"Host": host}).status_code == 200      # must fire
...
def test_a_host_we_could_not_read_is_a_different_arm_from_one_we_read_and_refused(app):
    ...
    c = _client(app)
    unreadable, refused = c.get("/", headers={"Host": "evil.com@127.0.0.1:8765"}), c.get("/", headers={"Host": "example.test"})
...
# The six arms of `_enforce`: the code, the `details` keys it guarantees, and the request that reaches it.
ARMS = [
    ("undetermined_host", {"host_header_present", "host_header", "hint"}, "get", {"Host": "evil.com@127.0.0.1"}),
    ("host_not_allowed", {"host", "hint"}, "get", {"Host": "example.test"}),
    ("cross_site_fetch", {"sec_fetch_site"}, "post", {"Sec-Fetch-Site": "cross-site"}),
    ("opaque_origin", {"origin", "host"}, "post", {"Origin": "null"}),
    ("origin_mismatch", {"origin", "host"}, "post", {"Origin": "http://example.test"}),
    ("missing_request_token", set(), "post", None),
]
...
def test_every_arm_has_its_own_code_and_the_family_base_is_raised_by_none():
    ...
    assert all(STATUS_BY_CODE[code] == 403 for code in codes + ["cross_site_request"])


@pytest.mark.parametrize("code, keys, method, headers", ARMS, ids=[a[0] for a in ARMS])
def test_each_arm_carries_exactly_the_details_shape_its_code_promises(code, keys, method, headers, raw_client, monkeypatch):
    ...
    if method == "get":
        ...
        raw_client.post("/sessions", data=_CREATE, headers={CSRF_HEADER: csrf_token(), **headers} if headers else {})
    exc = raised.get("exc")
    assert exc is not None and exc.code == code, f"no {code} refusal was raised"
...
def test_a_legitimate_post_raises_no_arm_at_all(client):
    ...
    r = client.post("/sessions", data={**_CREATE, "request_text": "A leave approval system."},
                    headers={"Origin": "http://127.0.0.1:8765", "Sec-Fetch-Site": "same-origin"}, follow_redirects=False)
    assert r.status_code == 303
...
def test_the_provider_probe_reads_either_credential_name(monkeypatch, variable, present):
    ...
    if not present:
        assert status.available is False and "ANTHROPIC_API_KEY" in status.reason and "ANTHROPIC_AUTH_TOKEN" in status.reason
