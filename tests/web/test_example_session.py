...
def _seed(client):
    ...
    assert r.status_code == 303 and r.headers["location"].startswith("/sessions/"), r.text[:400]
...
def _rows():
    return {r["slug"]: r for r in session_list(SessionService())}


# ── the affordance is on the page a keyless visitor lands on ──────────────────


def test_a_keyless_workspace_offers_a_route_into_the_example_empty_or_not(client):
    """The issue proposed showing this only on an empty workspace."""
    empty = client.get("/").text
    assert "Set ANTHROPIC_API_KEY" in empty and "Nothing here yet" in empty   # must fire: this really is the keyless state
    assert 'action="/sessions/example"' in empty
    _ordinary()
    populated = client.get("/").text
    assert "a-real-request" in populated and 'action="/sessions/example"' in populated


def test_seeding_is_refused_without_the_cross_site_token(raw_client):
    """Same guard as every other POST in this app."""
    assert raw_client.post("/sessions/example", follow_redirects=False).status_code == 403
...
def test_one_click_yields_a_browsable_session_through_the_validated_path(client):
    """A real session, not a hand-written directory; its revision claims no provider it did not use (invariant 6)."""
    slug = _seed(client)
    ...
    svc = SessionService()
    assert svc.meta(slug).current_revision == 1 and (canonical_dir(slug) / "revisions" / "0001-model.json").exists()
    status = svc.status(slug)
    assert status["questions"] and status["readiness"]["ready"] in (True, False)
    record = svc.meta(slug).revisions[-1]
...
def test_the_example_names_itself_in_the_listing_and_on_its_own_page_beside_a_real_session(client):
    ...
    ordinary, slug = _ordinary(), _seed(client)
    assert _rows()[slug]["is_example"] is True and _rows()[ordinary]["is_example"] is False   # must fire
    assert EXAMPLE_BADGE in client.get("/").text
    own = client.get(f"/sessions/{slug}").text
    assert EXAMPLE_BADGE in own and "bundled" in own.lower() and "no API key" in own
...
def test_the_example_is_recognised_by_what_it_asks_not_by_the_name_it_landed_under(client):
    ...
    slug = _seed(client)
    assert slug != EXAMPLE_SLUG and _rows()[slug]["is_example"] is True and _rows()[EXAMPLE_SLUG]["is_example"] is False
...
def test_the_bundled_brief_is_read_rather_than_restated():
    ...
    assert request.startswith("Look, the whole event thing is chaos") and "# Request" not in request and ">" not in request
    assert set(example_proposal()) >= {"model", "questions", "summary"}
    assert example_brief().startswith("# Decision Brief") and "One word, two different problems" in example_brief()
...
def test_one_click_also_seeds_the_decision_brief_no_key_needed(client):
    ...
    assert r.status_code == 200 and "Decision Brief" in r.text and "One word, two different problems" in r.text
...
def test_a_second_click_does_not_reseed_or_duplicate_the_brief(client, monkeypatch):
    """Idempotent on identity (invariant 11), not a fresh write every time (#428); a reader's own brief is never overwritten."""
    calls = []
    real_save = ArtifactService.save
    monkeypatch.setattr(ArtifactService, "save", lambda self, *a, **k: calls.append(a) or real_save(self, *a, **k))
    first = _seed(client)
    ...
    assert len([c for c in calls if c[:2] == (first, "brief")]) == 1, f"one save('brief') across two clicks expected: {calls}"
...
def test_seeding_the_brief_holds_the_lock_across_the_check_and_the_save(monkeypatch):
    """#428 review finding: the check-then-act on the saved brief runs under the session lock; the service alone suffices."""
    ...
    def _spying_list(self, slug):
        ...
        seen["depth"] = getattr(_held_locks, "depths", {}).get(key, 0)
        return real_list(self, slug)
    monkeypatch.setattr(ArtifactService, "list", _spying_list)
    slug = seed_example(sessions)   # the first click, the only one that reaches this gate
    assert sessions.meta(slug).current_revision == 1
    assert seen.get("depth", 0) > 0, "artifacts.list() ran with the lock NOT held, or was never called"
