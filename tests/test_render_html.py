...
def test_a_marker_inside_a_code_span_is_shown_rather_than_obeyed():
    """A backtick span is the one place these markers are meant to be seen."""
...
def test_an_underscore_inside_a_word_is_not_emphasis():
    """`business_rules` and `config_vs_custom` appear in this product's own prose."""
...
def test_a_blank_line_still_starts_a_new_paragraph():
    """Must fire for the test above. Turning every break into a `<br>` would also be satisfied by a renderer
    that emitted one giant paragraph for the whole document."""
...
def test_an_escaped_pipe_comes_back_as_a_pipe():
    """`_cell()` writes a literal pipe as a backslash-pipe so it cannot close the cell."""
...
def test_a_pipe_block_with_no_header_degrades_instead_of_crashing():
    ...
    # must fire: the surrounding document is still rendered, so this is not asserting about a renderer that gave up on the whole file.
...
def _unescape(html: str) -> str:
    """Entities back to characters, so a test can ask what the reader sees rather than how it is spelled on
    the wire."""
...
# The content is written by a language model and lives in a file the user can edit.
...
def test_markup_smuggled_inside_every_construct_is_still_escaped():
    """One arm per block type, because escaping is applied per text run and a construct that builds its own
    tags is exactly where a run gets forgotten."""
...
def test_an_attribute_break_out_cannot_reach_an_attribute():
    """There is no place in this output where document text lands inside an attribute."""
...
def test_an_unclosed_fence_does_not_swallow_the_document():
    """A construct outside the dialect degrades to escaped text, the same as the `<pre>` block it replaced,
    and never worse -- it must not consume everything after it."""
...
def test_a_paragraph_stops_at_the_heading_that_follows_it():
    """The hazard `_is_block_start` is actually written for."""
...
#
# `render/` was the one package directory outside `[tool.pyright]`'s scope.
...
def test_every_inline_markup_branch_is_a_named_group_with_a_tag():
    ...
    # must fire: the pattern really is the three-branch one the claim is about.
...
def test_every_inline_marker_still_names_the_group_that_matched_it():
    """The runtime half of the claim above: `lastgroup` naming a group is what makes the `_INLINE_TAGS` lookup
    total."""
    ...
    # The code-first ordering holds inside the span, and `a_b_c` keeps its underscores.
...
def test_the_two_list_markers_between_them_cover_every_line_the_collector_gathers():
    """The must-fire half: the assert above must not be the only thing standing."""
