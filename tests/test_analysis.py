"""The driver: information_value = uncertainty × impact, and the readiness it feeds (#72)."""
...
def test_soft_slots_are_medium_or_high_and_unresolved():
    # Real slot ids: the model must speak the schema's vocabulary (padded slots stay empty/low → not soft). business_objects precedes business_rules in schema order, so soft comes back in that order.
    model = out({
...
def test_readiness_flags_a_missing_high_impact_slot_as_blocker():
    # The north-star guard: a required high-impact slot the model omitted entirely must NOT vanish from readiness.
...
def test_readiness_blocks_a_thin_high_impact_slot_even_when_explicit():
    # Provenance is not coverage: a high-impact slot stated in one word (completeness below the soft boundary) is NOT resolved, even if its confidence is explicit — it must still block, not read as confirmed.
    model = out({
...
def test_state_of_gives_testable_its_own_bucket():
    # #610: a reader must never mistake a named, deferred-to-test gap for a confirmed fact or a plain open question -- it is neither, so it is a fourth bucket, not folded into "unknown".
...
def test_readiness_does_not_block_on_a_slot_deferred_to_a_test():
    # #610: the state's whole point is that a known unknown deliberately deferred to a test does not keep the target persona (a builder) from ever reaching "ready".
    model = out({
...
def test_understanding_view_carries_a_to_test_group():
    # The groups dict every JSON/Web reader iterates.
    model = out({
        "business_rules": {"completeness": 0, "confidence": "testable", "impact": "high",
...
def test_the_four_slot_projections_all_read_from_one_schema_parse(monkeypatch):
    ...
    `model_schema.json` independently."""
