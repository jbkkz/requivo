"""The live probe against this checkout's own CLI, and #174/invariant 16: the module that reads the most
foreign content stays stdlib-only and survives a console that cannot encode what it read."""
import ast
import io
import os
import sys
...
def test_the_plugin_resolves_against_this_checkouts_cli():
    ...
    `tests/test_plugin.py` already asserts at the top level."""
    surface = cli_surface(sys.executable)
...
def test_a_strict_console_reports_a_real_finding_as_could_not_look_when_streams_are_not_hardened(
    ...
    """must fire. `main()`'s own blanket `except Exception` already turns an unhandled `UnicodeEncodeError`
    into could-not-look rather than letting it escape (#174)."""
