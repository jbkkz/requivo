"""#283: the malformed reply survives retry give-up. `_complete`'s give-up exit used to discard
the raw reply along with the retry loop's local state; the final reply that never validated is now
written to `.requivo/debug/` and named in the raised error.
