...
def test_a_requirement_with_no_lower_bound_is_refused(requirement):
    """Refused, not skipped. Skipping is how a dependency ends up outside the constraints file with the leg
    still green — the newest release installed, and a report that the floor was tested."""
...
def test_a_named_runtime_extra_that_is_gone_is_refused():
    """`RUNTIME_EXTRAS` names the extras a user installs."""
...
def test_one_name_with_two_different_floors_is_refused():
    """pip would resolve the contradiction by picking one, and the leg would report a floor nobody declared."""
    pyproject = {"project": {
...
def test_the_same_floor_declared_twice_is_not_a_contradiction():
    """The must-not-fire half: `web` and `dev` legitimately restate the same requirement."""
    pyproject = {"project": {
        "dependencies": ["jinja2>=3.1,<4"],
...
def test_the_real_manifest_yields_every_runtime_dependency():
    """Named against the manifest rather than a count, so adding a dependency does not fail this test while
    *dropping* one from the floor set still does."""
...
def test_the_dev_toolchain_is_not_floored():
    """Scope is the promise a *user's* resolver has to satisfy."""
    pins = dict(line.split("==") for line in constraints(_real_pyproject()))
...
def test_every_runtime_extra_named_here_exists_in_the_manifest():
    """The other direction of the same rule: an extra this script claims to cover but the manifest no longer
    declares is a promise about something that is not there."""
...
def test_every_extra_in_the_manifest_is_either_floored_or_excluded_on_record():
    """The direction `test_every_runtime_extra_named_here_exists_in_the_manifest` does not check (#425)."""
...
def test_verify_reports_a_dependency_that_is_not_installed_at_all():
    """Absent is not satisfied. A name pip never had to resolve."""
...
#
# The bare positional argument used to be "whatever args[0] is.
...
def test_an_unrecognised_flag_is_refused_not_written_as_a_path(tmp_path, monkeypatch, capsys):
    """A typo of --verify is the worse case named in the issue."""
    monkeypatch.chdir(tmp_path)
