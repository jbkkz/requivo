"""Which model id a call actually uses: the env-chain resolution (`current_model_name`, #268) and the
constructor-level override (#434) that lets one process run two models concurrently."""
import json
...
def test_current_model_name_prefers_requivo_model_when_both_are_set(monkeypatch):
    # The precedence case #268 exists to pin: a shell that already exports a generic MODEL for some other tool must not silently steer Requivo once REQUIVO_MODEL is set alongside it.
...
# `AnthropicProvider` used to resolve the model ambiently, per call, through `current_model_name()`.


def test_two_constructed_providers_record_and_price_independently(monkeypatch):
    """Acceptance criterion 1, first half: two `AnthropicProvider`s in one process, each built with its own
    model id, make independent calls and are billed at their own model's rate."""
...
def test_default_construction_is_byte_identical_to_before_434(monkeypatch):
    """Acceptance criterion 2: `AnthropicProvider()`/`AnthropicProvider(client)` with no `model=` keyword is
    pinned to the pre-#434 behaviour."""
    from requivo.providers.anthropic import AnthropicProvider
...
def test_generate_threads_the_constructed_model_too():
    """The issue's own direction says 'threaded into the completion calls' (plural)."""
    from requivo.providers.anthropic import AnthropicProvider
