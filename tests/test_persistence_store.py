"""The store root's own bookkeeping: the privacy `.gitignore` (#211, #320) and what a half-finished
`save_revision` leaves behind (#261)."""
...
def test_the_privacy_gitignore_is_written_once_and_never_restored(workspace):
    """`.requivo/` lands in the caller's workspace, which defaults to cwd."""
    ...
    # Deleted on purpose: the team wants these sessions committed.
    marker.unlink()
...
def test_no_store_directory_is_created_outside_ensure_store_dir():
    ...
    exempt = {
        # Creates the staging tree for a session in flight.
...
def _tear_first_apply(slug: str, *, after: int) -> dict:
    """The other shape, and a different arm of `check_session`."""
    svc = SessionService()
...
def test_a_crash_after_the_first_payload_write_still_reads_as_the_recorded_revision(workspace):
    ...
    # The claim: every read path still serves the content session.json accounts for.
    ...
    # The mechanism behind it, asserted separately so a failure above reads as the defect rather than as a rearranged implementation: the one write that landed went to `revisions/`, which no read path consults, and it carries the content of the revision that never completed.
...
def test_a_crash_in_the_very_first_apply_leaves_a_session_still_at_revision_zero(workspace):
    """The same two gaps on the revision 0 → 1 path, which is a *different* arm of `check_session` and the one
    `migrate_legacy` takes."""
    ...
    # And the second gap, which the reorder does not close.
