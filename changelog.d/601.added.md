- A first `requivo discover` with no explicit `--perimeter` now **routes**: one cheap provider call
  ...
  one plausibly fits (the discovery refuses, before any model is reasoned, and names the candidates
  to choose between with `--perimeter`); none clearly fits (a real answer, not a failure — the
  session continues under the default perimeter, named rather than silently assumed) (#601).
